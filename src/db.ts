...
function createSchema(database: Database.Database): void {
  database.exec(`
    ...
    CREATE TABLE IF NOT EXISTS autopilot_state (
      ...
      qa_context    TEXT NOT NULL DEFAULT '',
      enriched_plan TEXT NOT NULL DEFAULT ''
    );
    ...
    CREATE INDEX IF NOT EXISTS idx_scheduler_logs_task ON scheduler_logs(task_id, started_at DESC);
    CREATE INDEX IF NOT EXISTS idx_scheduler_logs_time ON scheduler_logs(started_at DESC);
...
export function decayMemories(): void {
  ...
  db.prepare(
    "UPDATE memories SET salience = salience * 0.98 WHERE created_at < ? AND sector != 'knowledge'",
  ).run(oneDayAgo);
  db.prepare("DELETE FROM memories WHERE salience < 0.1 AND sector != 'knowledge'").run();
}

export function getKnowledgeMemories(chatId: string): Memory[] {
  return db
    .prepare(
      "SELECT * FROM memories WHERE chat_id = ? AND sector = 'knowledge' ORDER BY created_at DESC",
    )
...
export function insertSchedulerLog(
  taskId: string,
  taskPrompt: string,
  status: 'success' | 'error',
  output: string,
  ...
  db.prepare(
    `INSERT INTO scheduler_logs (task_id, task_prompt, status, output, error, started_at, finished_at, duration_ms)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(taskId, taskPrompt.slice(0, 200), status, output.slice(0, 2000), error.slice(0, 2000), startedAt, finishedAt, durationMs);
}

export function getSchedulerLogs(limit = 50): SchedulerLog[] {
  return db
    .prepare('SELECT * FROM scheduler_logs ORDER BY started_at DESC LIMIT ?')
    .all(limit) as SchedulerLog[];
}

export function getSchedulerLogsByTask(taskId: string, limit = 20): SchedulerLog[] {
  return db
    .prepare('SELECT * FROM scheduler_logs WHERE task_id = ? ORDER BY started_at DESC LIMIT ?')
    .all(taskId, limit) as SchedulerLog[];
}

export function pruneSchedulerLogs(keepDays = 30): void {
  const cutoff = Math.floor(Date.now() / 1000) - (keepDays * 86400);
  db.prepare('DELETE FROM scheduler_logs WHERE started_at < ?').run(cutoff);
}
...
export function enqueueAutopilotTask(
  taskId: string,
  featureId: string,
  projectId: string,
  taskDesc: string,
  ...
  const now = Math.floor(Date.now() / 1000);
  const result = db.prepare(
    `INSERT INTO autopilot_queue (task_id, feature_id, project_id, task_desc, project_name, status, created_at)
     VALUES (?, ?, ?, ?, ?, 'pending', ?)`,
  ).run(taskId, featureId, projectId, taskDesc, projectName, now);
  return result.lastInsertRowid as number;
}

export function isTaskEnqueued(taskId: string): boolean {
  const row = db.prepare(
    `SELECT id FROM autopilot_queue WHERE task_id = ? AND status IN ('pending', 'running')`,
  ).get(taskId);
  return !!row;
}

export function getNextPendingTask(): AutopilotQueueItem | null {
  return (db.prepare(
    `SELECT * FROM autopilot_queue WHERE status = 'pending' ORDER BY created_at ASC LIMIT 1`,
  ).get() as AutopilotQueueItem | undefined) ?? null;
}

export function hasRunningTask(): boolean {
  const row = db.prepare(
    `SELECT id FROM autopilot_queue WHERE status = 'running' LIMIT 1`,
  ).get();
  return !!row;
}
...
export function getRunningProjectIds(): string[] {
  const rows = db.prepare(
    `SELECT DISTINCT project_id FROM autopilot_queue WHERE status = 'running'`,
...
export function countRunningTasks(): number {
  const row = db.prepare(
    `SELECT COUNT(*) as cnt FROM autopilot_queue WHERE status = 'running'`,
  ).get() as { cnt: number };
  return row.cnt;
}

export function getNextPendingTaskExcludingProjects(excludeProjectIds: string[]): AutopilotQueueItem | null {
  if (excludeProjectIds.length === 0) return getNextPendingTask();
  const placeholders = excludeProjectIds.map(() => '?').join(',');
  return (db.prepare(
    `SELECT * FROM autopilot_queue WHERE status = 'pending' AND project_id NOT IN (${placeholders}) ORDER BY created_at ASC LIMIT 1`,
  ).get(...excludeProjectIds) as AutopilotQueueItem | undefined) ?? null;
}

export function getRunningFeatureIds(): string[] {
  const rows = db.prepare(
    `SELECT DISTINCT feature_id FROM autopilot_queue WHERE status = 'running'`,
  ).all() as { feature_id: string }[];
  return rows.map((r) => r.feature_id);
}

export function getNextPendingTaskExcludingFeatures(excludeFeatureIds: string[]): AutopilotQueueItem | null {
  if (excludeFeatureIds.length === 0) return getNextPendingTask();
  const placeholders = excludeFeatureIds.map(() => '?').join(',');
  return (db.prepare(
    `SELECT * FROM autopilot_queue WHERE status = 'pending' AND feature_id NOT IN (${placeholders}) ORDER BY created_at ASC LIMIT 1`,
  ).get(...excludeFeatureIds) as AutopilotQueueItem | undefined) ?? null;
}

export function resetOrphanedRunningTasks(): number {
  const result = db.prepare(
    `UPDATE autopilot_queue SET status = 'pending', started_at = NULL WHERE status = 'running'`,
  ).run();
  return result.changes;
}

export function markAutopilotTaskRunning(id: number): void {
  const now = Math.floor(Date.now() / 1000);
  db.prepare(
    `UPDATE autopilot_queue SET status = 'running', started_at = ? WHERE id = ?`,
  ).run(now, id);
}

export function markAutopilotTaskDone(id: number, output: string, commitSha: string, verificationStatus = ''): void {
  const now = Math.floor(Date.now() / 1000);
  db.prepare(
    `UPDATE autopilot_queue SET status = 'done', completed_at = ?, output = ?, commit_sha = ?, verification_status = ? WHERE id = ?`,
  ).run(now, output.slice(0, 5000), commitSha, verificationStatus, id);
}

export function markAutopilotTaskFailed(id: number, error: string): void {
  const now = Math.floor(Date.now() / 1000);
  db.prepare(
    `UPDATE autopilot_queue SET status = 'failed', completed_at = ?, error = ? WHERE id = ?`,
  ).run(now, error.slice(0, 2000), id);
}

export function getAutopilotQueue(status?: string): AutopilotQueueItem[] {
  if (status) {
    return db.prepare(
      `SELECT * FROM autopilot_queue WHERE status = ? ORDER BY created_at DESC`,
    ).all(status) as AutopilotQueueItem[];
  }
  return db.prepare(
    `SELECT * FROM autopilot_queue ORDER BY created_at DESC`,
  ).all() as AutopilotQueueItem[];
}

export function retryAutopilotTask(id: number): void {
  db.prepare(
    `UPDATE autopilot_queue SET status = 'pending', started_at = NULL, completed_at = NULL, error = '' WHERE id = ?`,
  ).run(id);
}

export function deleteAutopilotTask(id: number): void {
  db.prepare('DELETE FROM autopilot_queue WHERE id = ?').run(id);
}
...
export function getAutopilotState(projectId: string): AutopilotState | null {
  return db.prepare('SELECT * FROM autopilot_state WHERE project_id = ?').get(projectId) as AutopilotState | null;
}

export function upsertAutopilotState(projectId: string, status: string): void {
  db.prepare(
    `INSERT INTO autopilot_state (project_id, status) VALUES (?, ?)
     ON CONFLICT(project_id) DO UPDATE SET status = ?`,
  ).run(projectId, status, status);
}

export function setAutopilotStatus(projectId: string, status: string): void {
  db.prepare('UPDATE autopilot_state SET status = ? WHERE project_id = ?').run(status, projectId);
}

export function appendClarificationQA(projectId: string, question: string, answer: string): void {
  const state = getAutopilotState(projectId);
  const qa: Array<{ q: string; a: string }> = state?.qa_context ? JSON.parse(state.qa_context) : [];
  const existing = qa.findIndex((item) => item.q === question);
  if (existing !== -1) {
    qa[existing].a = answer;
  } else {
    qa.push({ q: question, a: answer });
  }
  db.prepare('UPDATE autopilot_state SET qa_context = ? WHERE project_id = ?').run(JSON.stringify(qa), projectId);
}

export function getClarificationContext(projectId: string): Array<{ q: string; a: string }> {
  const state = getAutopilotState(projectId);
  if (!state?.qa_context) return [];
  return JSON.parse(state.qa_context);
}

export function setEnrichedPlan(projectId: string, plan: string): void {
  db.prepare('UPDATE autopilot_state SET enriched_plan = ?, status = ? WHERE project_id = ?').run(plan, 'enriched', projectId);
}

export function markProjectReady(projectId: string): void {
  db.prepare("UPDATE autopilot_state SET status = 'ready' WHERE project_id = ?").run(projectId);
}
