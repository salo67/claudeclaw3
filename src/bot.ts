...
import {
  ...
  resolvePhase, formatProjectList, formatProjectDetail,
  type KanbanProject, type KanbanFeature, type KanbanTask,
} from './kanban.js';
import { createAdvisorThread, getOrCreateAdvisorThread, sendAdvisorMessage } from './advisor.js';
...
// Advisor state per Telegram chat — stores active thread ID
const advisorState = new Map<string, string>(); // chatId -> threadId

// Autopilot clarification state per Telegram chat
interface AutopilotClarifyState {
  mode: 'clarifying' | 'awaiting_confirmation';
  projectId: string;
  projectName: string;
}
const autopilotState = new Map<string, AutopilotClarifyState>(); // chatId -> state

/** Set autopilot clarification state from outside (used by index.ts) */
export function setAutopilotClarifyState(chatId: string, state: AutopilotClarifyState): void {
  autopilotState.set(chatId, state);
}

// Kanban state per Telegram chat
interface KanbanStateList { mode: 'list'; projects: KanbanProject[] }
interface KanbanStateProject {
  mode: 'project';
  project: KanbanProject;
  features: KanbanFeature[];
  tasksByFeature: Map<string, KanbanTask[]>;
}
type KanbanState = KanbanStateList | KanbanStateProject;
const kanbanState = new Map<string, KanbanState>();

// ── Quick-capture: detect notes & journal entries before sending to Claude ──

const NOTE_PATTERNS = [
  /^nota[:\s]/i, /^note[:\s]/i, /^apunta[:\s]/i, /^apúnta(me)?[:\s]/i,
  /^recuérdame que\b/i, /^recordar(me)? que\b/i, /^quiero recordar\b/i,
  /^anota[:\s]/i, /^anóta(me)?[:\s]/i, /^guarda(me)? (esto|que)\b/i, /^para recordar[:\s]/i,
];

const JOURNAL_PATTERNS = [
  /^reflexi[oó]n[:\s]/i, /^journal[:\s]/i, /^diario[:\s]/i,
  /^hoy (me siento|fue|estuvo|pens[eé]|reflexion[eé]|aprend[ií])\b/i,
  /^estuve pensando\b/i, /^me di cuenta\b/i, /^aprend[ií] (que|algo)\b/i, /^me siento\b/i,
];

function extractNoteContent(text: string): string {
  return text
    .replace(/^(nota|note|apunta|apúntame|anota|anótame|guárdame esto|guárdame que|para recordar)[:\s]+/i, '')
    .replace(/^(recuérdame que|recordarme que|quiero recordar)\s+/i, '')
    .trim();
}

function extractJournalContent(text: string): string {
  return text.replace(/^(reflexi[oó]n|journal|diario)[:\s]+/i, '').trim();
}
...
async function tryQuickCapture(ctx: Context, text: string): Promise<boolean> {
  const trimmed = text.trim();
  if (JOURNAL_PATTERNS.some((p) => p.test(trimmed))) {
    try {
      const content = extractJournalContent(trimmed);
      await addJournalEntry(content);
      await ctx.reply('📓 Anotado en el journal.');
      logger.info({ len: content.length }, 'Quick-capture: journal entry');
      return true;
    } catch (err) {
      logger.error({ err }, 'Quick-capture journal failed');
      await ctx.reply('No pude guardar en el journal. Verifica que el API esté corriendo.');
      return true;
    }
  }
  if (NOTE_PATTERNS.some((p) => p.test(trimmed))) {
    try {
      const content = extractNoteContent(trimmed);
      const title = deriveNoteTitle(content);
      await createNote(title, content);
      await ctx.reply(`📝 Nota guardada: ${title}`);
      logger.info({ title, len: content.length }, 'Quick-capture: note');
      return true;
    } catch (err) {
      logger.error({ err }, 'Quick-capture note failed');
      await ctx.reply('No pude guardar la nota. Verifica que el API esté corriendo.');
      return true;
    }
  }
  return false;
...
export function createBot(): Bot {
  ...
  // ── Fork features: advisor, kanban, journal, notes ──────────────────

  bot.command('advisor', async (ctx) => {
    const chatIdStr = ctx.chat!.id.toString();
    if (!isAuthorised(ctx.chat!.id)) return;
    const args = (ctx.message?.text || '').replace(/^\/advisor\s*/i, '').trim().toLowerCase();
    if (args === 'new') {
      try {
        await sendTyping(ctx.api, ctx.chat!.id);
        const thread = await createAdvisorThread();
        advisorState.set(chatIdStr, thread.id);
        waState.delete(chatIdStr); slackState.delete(chatIdStr);
        await ctx.reply('Advisor ON (new thread). Send /advisor to switch back to Claude.');
      } catch (err) {
        logger.error({ err }, '/advisor new failed');
        await ctx.reply('Could not create new advisor thread.');
      }
      return;
    }
    if (advisorState.has(chatIdStr)) {
      advisorState.delete(chatIdStr);
      await ctx.reply('Advisor OFF. Back to Claude.');
      return;
    }
    try {
      await sendTyping(ctx.api, ctx.chat!.id);
      ...
      advisorState.set(chatIdStr, thread.id);
      waState.delete(chatIdStr); slackState.delete(chatIdStr);
      const isResumed = thread.last_message ? ' (resuming last thread)' : '';
      await ctx.reply(`Advisor ON${isResumed}. Send /advisor to switch back, /advisor new for fresh thread.`);
    } catch (err) {
      logger.error({ err }, '/advisor command failed');
      await ctx.reply('Could not connect to advisor. Make sure the API is running.');
    }
  });
  ...
  bot.command('projects', async (ctx) => {
    ...
    try {
      ...
      kanbanState.set(chatIdStr, { mode: 'list', projects });
      waState.delete(chatIdStr); slackState.delete(chatIdStr); advisorState.delete(chatIdStr);
      await ctx.reply(formatProjectList(projects), { parse_mode: 'HTML' });
    } catch (err) {
      logger.error({ err }, '/projects command failed');
      await ctx.reply('Could not load projects. Make sure the API is running.');
    }
  });
  ...
  bot.command('journal', async (ctx) => {
    ...
    const body = text.replace(/^\/journal\s*/i, '').trim();
    try {
      await sendTyping(ctx.api, ctx.chat!.id);
      if (!body) {
        const today = await getTodayEntry();
        if (today) {
          await ctx.reply(formatEntry(today));
        } else {
          const recent = await getRecentEntries(5);
          if (recent.length > 0) {
            await ctx.reply('No hay entrada de hoy. Recientes:\n\n' + formatEntryList(recent));
          } else {
            await ctx.reply('Journal vacio. Usa /journal <texto> para agregar una entrada.');
          }
        }
        return;
      }
      let mood: string | undefined;
      const moodMatch = body.match(/^:(great|good|neutral|stressed|bad)\s+/);
      if (moodMatch) mood = moodMatch[1];
      const content = moodMatch ? body.slice(moodMatch[0].length) : body;
      await addJournalEntry(content, mood);
      const moodEmoji: Record<string, string> = { great: '🔥', good: '😊', neutral: '😐', stressed: '😤', bad: '😞' };
      const moodStr = mood ? ` ${moodEmoji[mood]}` : '';
      await ctx.reply(`Anotado${moodStr} 📓`);
    } catch (err) {
      logger.error({ err }, '/journal command failed');
      await ctx.reply('No pude guardar en el journal. Verifica que el API este corriendo.');
    }
  });
  ...
  bot.command('nota', async (ctx) => {
    if (!isAuthorised(ctx.chat!.id)) return;
    const text = ctx.message?.text || '';
    const body = text.replace(/^\/nota\s*/i, '').trim();
    try {
      await sendTyping(ctx.api, ctx.chat!.id);
      if (!body) {
        const recent = await getRecentNotes(5);
        await ctx.reply(formatNoteList(recent));
        return;
      }
      const tagMatches = body.match(/#(\w+)/g);
      const tags = tagMatches ? tagMatches.map((t) => t.slice(1)).join(',') : '';
      const content = body.replace(/#\w+/g, '').replace(/\s{2,}/g, ' ').trim();
      const title = content.split(/\s+/).slice(0, 5).join(' ');
      const note = await createNote(title, content, tags || undefined);
      await ctx.reply(`📝 Nota guardada: ${note.title}`);
    } catch (err) {
      logger.error({ err }, '/nota command failed');
      await ctx.reply('No pude guardar la nota. Verifica que el API esté corriendo.');
    }
  });
  ...
  bot.on('message:text', async (ctx) => {
    ...
    const advisorThreadId = advisorState.get(chatIdStr);
    if (advisorThreadId) {
      await sendTyping(ctx.api, ctx.chat!.id);
      const typingInterval = setInterval(() => void sendTyping(ctx.api, ctx.chat!.id), TYPING_REFRESH_MS);
      try {
        const response = await sendAdvisorMessage(advisorThreadId, text);
        clearInterval(typingInterval);
        const formatted = formatForTelegram(response || 'No response.');
        for (const part of splitMessage(formatted)) {
          await ctx.reply(part, { parse_mode: 'HTML' });
        ...
        clearInterval(typingInterval);
        logger.error({ err }, 'Advisor message failed');
        await ctx.reply('Advisor error. Try again or /advisor to switch back.');
      }
      return;
    }

    // ── Autopilot clarification state machine ──────────────────────
    const apState = autopilotState.get(chatIdStr);
    if (apState) {
      const { handleClarificationReply, handleEnrichmentConfirmation } = await import('./autopilot.js');
      if (apState.mode === 'clarifying') {
        await handleClarificationReply(apState.projectId, text);
        const { getAutopilotState: getApState } = await import('./db.js');
        const dbState = getApState(apState.projectId);
        if (dbState && dbState.status !== 'clarifying') autopilotState.delete(chatIdStr);
        return;
      }
      if (apState.mode === 'awaiting_confirmation') {
        if (/^(ok|go|listo|dale|start|si|yes)$/i.test(text.trim())) {
          handleEnrichmentConfirmation(apState.projectId);
          autopilotState.delete(chatIdStr);
          await ctx.reply(`Autopilot activado para ${apState.projectName}. Las waves empezaran en el proximo heartbeat.`);
        } else if (/^(no|cancel|cancelar)$/i.test(text.trim())) {
          autopilotState.delete(chatIdStr);
          await ctx.reply('Autopilot cancelado. Puedes ajustar features en el dashboard y reactivar.');
        }
        return;
      }
    }
    ...
    const kbState = kanbanState.get(chatIdStr);
    if (kbState) {
      if (/^b(ack)?$/i.test(text.trim())) {
        if (kbState.mode === 'project') {
          try {
            const projects = await listProjects();
            kanbanState.set(chatIdStr, { mode: 'list', projects });
            await ctx.reply(formatProjectList(projects), { parse_mode: 'HTML' });
          } catch (err) {
            logger.error({ err }, 'Kanban back failed');
            await ctx.reply('Error loading projects.');
          }
        } else {
          kanbanState.delete(chatIdStr);
          await ctx.reply('Kanban closed.');
        }
        return;
      }
      if (kbState.mode === 'list') {
        const num = extractSelectionNumber(text);
        if (num !== null && num >= 1 && num <= kbState.projects.length) {
          const proj = kbState.projects[num - 1];
          try {
            await sendTyping(ctx.api, ctx.chat!.id);
            const [fullProject, feats] = await Promise.all([getProject(proj.id), listFeatures(proj.id)]);
            const tasksByFeature = new Map<string, KanbanTask[]>();
            const taskResults = await Promise.all(feats.map((f) => listTasks({ feature_id: f.id })));
            feats.forEach((f, i) => tasksByFeature.set(f.id, taskResults[i]));
            kanbanState.set(chatIdStr, { mode: 'project', project: fullProject, features: feats, tasksByFeature });
            await ctx.reply(formatProjectDetail(fullProject, feats, tasksByFeature), { parse_mode: 'HTML' });
          } catch (err) {
            logger.error({ err }, 'Kanban open project failed');
            await ctx.reply('Error opening project.');
          }
          return;
        }
        kanbanState.delete(chatIdStr);
      ...
      if (kbState.mode === 'project') {
        const moveMatch = text.match(/^m\s+(\d+)\s+(\S+)/i);
        if (moveMatch) {
          const fIdx = parseInt(moveMatch[1]) - 1;
          const phase = resolvePhase(moveMatch[2]);
          if (phase && fIdx >= 0 && fIdx < kbState.features.length) {
            const feat = kbState.features[fIdx];
            try {
              await updateFeature(feat.id, { phase });
              feat.phase = phase;
              await ctx.reply(`Moved "<b>${escapeHtml(feat.description)}</b>" to ${phase.replace('_', ' ')}`, { parse_mode: 'HTML' });
              const feats = await listFeatures(kbState.project.id);
              const tasksByFeature = new Map<string, KanbanTask[]>();
              const taskResults = await Promise.all(feats.map((f) => listTasks({ feature_id: f.id })));
              feats.forEach((f, i) => tasksByFeature.set(f.id, taskResults[i]));
              kanbanState.set(chatIdStr, { ...kbState, features: feats, tasksByFeature });
              await ctx.reply(formatProjectDetail(kbState.project, feats, tasksByFeature), { parse_mode: 'HTML' });
            } catch (err) {
              logger.error({ err }, 'Kanban move feature failed');
              await ctx.reply('Error moving feature.');
            }
          } else {
            await ctx.reply('Invalid feature number or phase. Phases: backlog(b), todo(t), in_progress(ip), done(d)');
          }
          return;
        }
        const doneMatch = text.match(/^d\s+(\d+)/i);
        if (doneMatch) {
          const tIdx = parseInt(doneMatch[1]) - 1;
          const allTasks: KanbanTask[] = [];
          for (const f of kbState.features) {
            const fTasks = kbState.tasksByFeature.get(f.id) || [];
            allTasks.push(...fTasks.filter((t) => !t.completed));
          }
          if (tIdx >= 0 && tIdx < allTasks.length) {
            try {
              await updateTask(allTasks[tIdx].id, { completed: true });
              await ctx.reply(`✓ Done: ${escapeHtml(allTasks[tIdx].description)}`, { parse_mode: 'HTML' });
            } catch (err) {
              logger.error({ err }, 'Kanban mark task done failed');
              await ctx.reply('Error marking task done.');
            }
          } else {
        ...
        const newMatch = text.match(/^new\s+(.+)/is);
        if (newMatch) {
          try {
            const feat = await createFeature(kbState.project.id, newMatch[1].trim());
            await ctx.reply(`Created feature: <b>${escapeHtml(feat.description)}</b>`, { parse_mode: 'HTML' });
          } catch (err) {
            logger.error({ err }, 'Kanban create feature failed');
            await ctx.reply('Error creating feature.');
          }
          return;
        }
        kanbanState.delete(chatIdStr);
    ...
    // Clear WA/Slack/Kanban state and pass through to Claude
    if (state) waState.delete(chatIdStr);
    if (slkState) slackState.delete(chatIdStr);
    kanbanState.delete(chatIdStr);

    // Quick-capture: detect notes / journal entries before sending to Claude
    const captured = await tryQuickCapture(ctx, text);
    if (captured) return;
